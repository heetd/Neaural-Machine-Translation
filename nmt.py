...
class LanguageDict(): # sents will be list of lists of tokens(words)
  def __init__(self, sents):
    word_counter = collections.Counter(tok.lower() for sent in sents for tok in sent)

    self.vocab = []
    self.vocab.append('<pad>') #zero paddings # id wpuld be 0 in word2ids
    self.vocab.append('<unk>') # id wpuld be 1 in word2ids
...
def load_dataset(source_path,target_path, max_num_examples=30000):
  ...
  target_sents = [[tok.lower() for tok in sent.strip().split(' ')] for sent in target_lines]#[["I","am","good"],["how","are"]]
    # for the target sentences, add <start> and <end> tokens to each sentence 
  for sent in target_sents:
    sent.append('<end>')
    sent.insert(0,'<start>') #[['<start>',"I","am","good",'<end>'],['<start>',"how","are",'<end>']]
  ...
  eos = target_lang_dict.word2ids['<end>'] # create new key '<end>' and assign eos as value
...
class AttentionLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    # the dimension of decoder_outputs is now [batch_size, max_target_sent_len, hidden_size]
...
class NmtModel(object):
  ...
  def eval(self, dataset):
    ...
    for i in range(5):
        print("### SAMPLE ",i+1,"###")
        #print("THIS IS REFERENCE: ",references[i])
        #print("THIS IS CANDIDATES:" ,candidates[i])        
        print("This is the translated sentence: ")
        print(' '.join(candidates[i]))
        print()
        print("This is the target sentence: ")
